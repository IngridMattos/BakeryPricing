...
WORKDIR /src
COPY ["PrecificaçãoConfeitaria.csproj", "."]
RUN dotnet restore "./PrecificaçãoConfeitaria.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./PrecificaçãoConfeitaria.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Esta fase é usada para publicar o projeto de serviço a ser copiado para a fase final
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./PrecificaçãoConfeitaria.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=true
...
ENTRYPOINT ["PrecificaçãoConfeitaria.exe"]